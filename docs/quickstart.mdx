...
Summary: ${ctx.outputMaybe("research", { nodeId: "research" })?.summary ?? ""}
Key Points: ${JSON.stringify(ctx.outputMaybe("research", { nodeId: "research" })?.keyPoints ?? [])}`}
    </Task>
  </Workflow>
));
```
...
- **`createSmithers()`** takes a record of named schemas and returns a `Workflow` component, a `smithers()` builder function, an `outputs` object with typed Zod schema references, and a configured database. You never touch SQLite or Drizzle directly.
...
- **`ctx.outputMaybe("research", { nodeId: "research" })`** retrieves the persisted output of the `research` task, or `undefined` if it has not completed yet. Since the JSX tree is rendered before tasks execute, always use `outputMaybe` with optional chaining (`?.`) for downstream task prompts. `<Workflow>` sequences its children by default, so `report` only runs after `research` finishes -- by that point, the tree re-renders and the value is available.
...
2. **Schedule** -- The engine walks the plan tree and determines that `research` is runnable (no dependencies) and `report` is blocked (it depends on `research` completing). On this first render, `ctx.outputMaybe("research", ...)` returns `undefined`, so the `report` task's prompt uses the fallback values.
...
4. **Re-render** -- The tree re-renders. This time `ctx.outputMaybe("research", { nodeId: "research" })` returns the persisted row, so the `report` task's prompt is fully resolved.
...
| **Context (`ctx`)** | Provides access to `input`, completed output rows via `outputMaybe`/`latest`, `runId`, and `iteration`. Passed to the `smithers()` build function. |
...
| **Schema Registry** | When you pass Zod schemas to `createSmithers()`, the framework auto-creates SQLite tables and returns an `outputs` object. Use `output={outputs.research}` to bind a task to its schema. |

## Next Steps

- [Execution Model](/concepts/execution-model) -- Deep dive into the render-schedule-execute loop.
- [Task Component](/components/task) -- All `<Task>` props: `retries`, `needsApproval`, `skipIf`, `timeoutMs`, `continueOnFail`.
- [Ralph Component](/components/ralph) -- Iteration loops with `until`, `maxIterations`, and `onMaxReached`.
- [Context](/concepts/context) -- Full `SmithersCtx` API: `output`, `outputMaybe`, `latest`, `latestArray`, `iterationCount`.
- [Tools](/integrations/tools) -- Built-in sandboxed tools (`read`, `edit`, `bash`, `grep`, `write`) for code-editing agents.
- [Examples](/examples/hello-world) -- More workflow patterns: dynamic plans, approval gates, multi-agent review, and Ralph loops.
