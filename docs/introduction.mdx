...
**Durable SQLite state.** Every task output is written to SQLite (via Drizzle ORM) keyed by `(runId, nodeId, iteration)`. Durable checkpoints, attempts, approvals, events, and frames are persisted there so a crashed process can resume from the last committed state.
...
- [Installation](/installation) -- Install Smithers and configure your project.
- [Quickstart](/quickstart) -- Build and run your first workflow in under 5 minutes.
- [Execution Model](/concepts/execution-model) -- Understand the render-execute-persist loop in depth.
