...
Smithers persists its durable workflow state -- inputs, outputs, execution metadata, approvals, events, frames, and cache -- in a single SQLite database managed through Drizzle ORM. The process still keeps transient runtime helpers in memory while a run is active. There are two ways to set up the data layer depending on how much control you need.
