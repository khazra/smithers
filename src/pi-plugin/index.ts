...
export async function runWorkflow(args: { workflowPath: string; input: unknown; runId?: string; baseUrl?: string; apiKey?: string }) {
...
export async function resume(args: { workflowPath: string; runId: string; baseUrl?: string; apiKey?: string }) {
  return post("/v1/runs", { workflowPath: args.workflowPath, runId: args.runId, resume: true }, { baseUrl: args.baseUrl, apiKey: args.apiKey });
}

export async function approve(args: { runId: string; nodeId: string; iteration?: number; note?: string; baseUrl?: string; apiKey?: string }) {
...
export async function deny(args: { runId: string; nodeId: string; iteration?: number; note?: string; baseUrl?: string; apiKey?: string }) {
...
export async function* streamEvents(args: { runId: string; baseUrl?: string; apiKey?: string }): AsyncIterable<SmithersEvent> {
...
export async function getStatus(args: { runId: string; baseUrl?: string; apiKey?: string }) {
  const base = args.baseUrl ?? DEFAULT_BASE;
...
export async function getFrames(args: { runId: string; tail?: number; baseUrl?: string; apiKey?: string }) {
  const base = args.baseUrl ?? DEFAULT_BASE;
...
export async function cancel(args: { runId: string; baseUrl?: string; apiKey?: string }) {
  return post(`/v1/runs/${args.runId}/cancel`, {}, { baseUrl: args.baseUrl, apiKey: args.apiKey });
}

export async function listRuns(args: { limit?: number; status?: string; baseUrl?: string; apiKey?: string } = {}) {
  const base = args.baseUrl ?? DEFAULT_BASE;
